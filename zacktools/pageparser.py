...
def parse(page, domain='', get_more_page=True, contact_page=None, about_page=None):
    ...
    if allLinks:
        ...
        if not domain:
            domains = Counter([toDomain(
                l) for l in allLinks if 'facebook' not in l and 'twitter' not in l and 'linkedin' not in l]).most_common(1)
            if domains and len(domains[0])> 0:
                domain = domains[0][0]
            else:
                domain = ''
        else:
...
if __name__ == '__main__':
    ...
    # import kumihotools
    with open('/home/zdai/zacktools/testhmtl.txt') as f:
        htmltext = f.read()
    print(parse(htmltext))
